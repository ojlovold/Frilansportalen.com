...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const blobRes = await fetch(`https://api.github.com/repos/${repo}/git/blobs`, {
      ...
      body: JSON.stringify({ content: Buffer.from(innhold).toString('base64'), encoding: 'base64' }),
    })
    ...
    const treeRes = await fetch(`https://api.github.com/repos/${repo}/git/trees`, {
      ...
      body: JSON.stringify({
        ...
        tree: [{ path: filnavn, mode: '100644', type: 'blob', sha: blob.sha }],
      }),
    })
    ...
    console.error(err)
    return res.status(500).json({ error: 'Push feilet' })
  }
}
